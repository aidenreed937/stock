...
def test_backfill_daily_multi_worker():
    ...
    with (
        patch("stock_data.backfill.create_pipeline", return_value=mock_pipeline),
        patch(
        ...
        patch("stock_data.backfill.DataUpdateScheduler.is_data_ready", return_value=True),
    ):
...
def test_backfill_per_symbol_without_symbol_does_not_use_endpoint_as_symbol() -> None:
    ...
    with patch("stock_data.backfill.create_pipeline", return_value=mock_pipeline):
...
def test_backfill_cli_main(monkeypatch):
    ...
    with patch("stock_data.backfill.HistoricalBackfiller", return_value=mock_backfiller):
        backfill_main()
...
"""
    ...
    with (
        ...
        patch("stock_data.backfill.HistoricalBackfiller", return_value=mock_backfiller),
    ):
        backfill_main()
...
def test_tushare_stock_endpoints_use_local_stock_basic_pool(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "stock_data.backfill._load_curated_symbol_pool",
...
def test_load_curated_symbol_pool_with_ts_code_column() -> None:
    from stock_data.backfill import _load_curated_symbol_pool
...
def test_planner_watchlist_on_per_day_endpoint_preserves_full_market() -> None:
    from stock_data.planner import BackfillPlanner
