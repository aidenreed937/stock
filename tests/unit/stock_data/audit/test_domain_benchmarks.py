...
def test_equity_benchmark_provider() -> None:
    ...
    with patch(
        "stock_data.audit.benchmarks.equity.get_trading_calendar", return_value=[date(2026, 8, 14)]
    ):
...
def test_industry_benchmark_provider() -> None:
    ...
    with patch(
        "stock_data.audit.benchmarks.industry.get_trading_calendar",
...
def test_index_benchmark_provider() -> None:
    mock_catalog = MagicMock()
    with patch(
        "stock_data.audit.benchmarks.index.get_trading_calendar", return_value=[date(2026, 8, 14)]
    ):
