...
from stock_data.governance.ops.migration import migrate_parquet
...
def test_backfill_acceptance_normalizes_compact_boundary_dates(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        report = accept_backfill(str(tmp_path), "adj_factor", date(2026, 8, 12), date(2026, 8, 13))
...
def test_backfill_acceptance_fails_on_internal_daily_gap(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        report = accept_backfill(str(tmp_path), "adj_factor", date(2026, 8, 12), date(2026, 8, 14))
...
def test_backfill_acceptance_ignores_statutory_holidays_from_trade_calendar(
    ...
    with patch(
        ...
        report = accept_backfill(str(tmp_path), "adj_factor", date(2023, 1, 20), date(2023, 1, 30))
...
def test_backfill_acceptance_handles_quarterly_report_periods(tmp_path: Path) -> None:
    ...
    report = accept_backfill(str(tmp_path), "income", date(2014, 1, 1), date(2026, 8, 12))
...
def test_endpoint_contract_rejects_duplicate_primary_keys() -> None:
    ...
    try:
        pipeline._validate_endpoint_frame(
            frame, __import__("datetime").date(2026, 8, 1), __import__("datetime").date(2026, 8, 1)
        )
