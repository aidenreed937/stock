...
def test_run_audit_dispatches_correctly() -> None:
    with (
        patch(
            "stock_data.governance.audit.master_audit.run_master_audit", return_value={"total": 100}
        ) as mock_master,
        patch("stock_data.governance.audit.master_audit.print_master_audit_summary"),
    ):
    ...
    with patch(
        "stock_data.governance.audit.backfill_acceptance.accept_backfill",
    ...
    with (
        patch(
            "stock_data.governance.audit.valuation_audit.run_daily_basic_audit", return_value={}
        ) as mock_val,
        patch(
            "stock_data.governance.audit.valuation_audit.run_sw_industry_audit", return_value={}
        ) as mock_sw,
    ):
        res = run_audit(
    ...
    with (
        patch(
            "stock_data.governance.audit.factor_audit.run_adj_factor_audit", return_value={}
        ) as mock_fac,
        patch(
            "stock_data.governance.audit.factor_audit.run_sw_daily_audit", return_value={}
        ) as mock_sw_d,
    ):
    ...
    with patch(
        "stock_data.governance.audit.moneyflow_audit.run_hk_hold_audit", return_value={}
    ) as mock_mf:
        res = run_audit(audit_type="moneyflow", data_source="tushare")
        mock_mf.assert_called_once()

    with patch(
        "stock_data.governance.audit.distribution_audit.run_distribution_audit",
        return_value={"status": "PASSED"},
    ) as mock_dist:
