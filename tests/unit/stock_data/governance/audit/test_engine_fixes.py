...
from stock_data.governance.audit.engine import UniversalAuditEngine
from stock_data.governance.audit.factor_audit import run_sw_daily_audit
...
def test_engine_false_positive_prevention() -> None:
    ...
    with patch(
        "stock_data.governance.audit.engine.resolve_benchmark_provider", return_value=mock_provider
    ):
...
def test_check_raw_curated_partition_flow(tmp_path) -> None:
    ...
    with (
        patch("stock_data.core.settings.data_settings.raw_data_dir", tmp_path / "raw"),
        patch("stock_data.core.settings.data_settings.curated_data_dir", tmp_path / "curated"),
    ):
...
def test_factor_audit_sw_daily_exact(tmp_path, monkeypatch) -> None:
    ...
    curated_root = tmp_path / "curated"
    monkeypatch.setattr(
        "stock_data.governance.audit.factor_audit.data_settings.curated_data_dir", curated_root
    )
...
def test_industry_benchmark_lixinger_empty_shells_filtered() -> None:
    ...
    from stock_data.governance.audit.benchmarks.industry import (
