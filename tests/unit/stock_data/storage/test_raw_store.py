...
def test_raw_storage_margin_deduplicates_mixed_date_formats(tmp_path: Path) -> None:
    ...
    assert len(loaded) == 2
    assert (
        normalized.group_by(["_trade_date", "exchange_id"])
        .len()
        .filter(pl.col("len") > 1)
        .is_empty()
    )
...
def test_raw_storage_replaces_incompatible_cache_with_recoverable_backup(
    ...
    fresh = pl.DataFrame({"industryCode": ["110000"], "stockCode": ["600519"], "market": ["CN"]})
    ...
    backup = (
        tmp_path / "lixinger" / "market=CN" / "sw_2021_constituents" / "data.legacy.bak.parquet"
    )
