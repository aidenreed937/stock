...
def test_watchlist_symbols_resolution() -> None:
    ...
    assert _watchlist_symbols("tushare", "index_daily", cfg_indices, {"index_daily"}) == [
        "000300.SH"
    ]
...
def test_resolve_calendar_dates_uses_fred_natural_calendar() -> None:
    ...
    natural_days = _resolve_calendar_dates(fetcher_mock, "fred", date(2024, 1, 5), date(2024, 1, 8))
...
def test_resolve_calendar_dates_uses_source_after_local_calendar_unavailable() -> None:
    ...
    with patch(
        "stock_data.pipeline.backfill.DataUpdateScheduler.get_trading_days", return_value=()
    ):
        dates = _resolve_calendar_dates(fetcher, "tushare", date(2024, 1, 5), date(2024, 1, 8))
...
def test_resolve_calendar_dates_fails_without_trusted_calendar() -> None:
    ...
    with (
        patch("stock_data.pipeline.backfill.DataUpdateScheduler.get_trading_days", return_value=()),
