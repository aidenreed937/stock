...
def test_clean_partition_daily_basic(tmp_path: Path) -> None:
    ...
    df_raw = pl.DataFrame({
        ...
        "total_mv": [2231700.0, 22317000000.0, 5000000000.0],  # 1: 万元, 2: 元(重复), 3: 元(仅截断存在)
        "circ_mv": [2231600.0, 22316000000.0, 4000000000.0],
        "close": [11.5, None, None],
        "pe": [4.8, None, None],
        "source_unit_note": [None, "Tushare daily_basic market values are normalized", "Tushare daily_basic market values are normalized"],
    })
