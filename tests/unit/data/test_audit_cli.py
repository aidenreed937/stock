...
def test_run_audit_dispatches_correctly() -> None:
    ...
    with patch("stock.data.audit.valuation_audit.run_daily_basic_audit", return_value={}) as mock_val, \
        ...
        res = run_audit(audit_type="valuation", data_source="tushare", target_date=date(2026, 8, 12))
        mock_val.assert_called_once()
        mock_sw.assert_called_once()

    with patch("stock.data.audit.factor_audit.run_adj_factor_audit", return_value={}) as mock_fac, \
...
def test_main_cli_dispatch() -> None:
    with patch("sys.argv", ["audit.py", "-t", "master", "-s", "tushare", "-d", "2026-08-12"]), \
...
def test_main_cli_error_exit() -> None:
    with patch("sys.argv", ["audit.py", "-t", "master"]), \
