...
from stock_cli.audit import main, run_audit


def test_run_audit_dispatches_correctly() -> None:
    with (
        patch(
            "stock_data.audit.master_audit.run_master_audit", return_value={"total": 100}
        ) as mock_master,
        patch("stock_data.audit.master_audit.print_master_audit_summary"),
    ):
    ...
    with patch(
        "stock_data.audit.reconciliation.run_audit", return_value={"integrity_rate": 100.0}
    ) as mock_recon:
    ...
    with patch(
        "stock_data.audit.backfill_acceptance.accept_backfill", return_value={"status": "PASSED"}
    ) as mock_acc:
    ...
    with (
        patch(
            "stock_data.audit.valuation_audit.run_daily_basic_audit", return_value={}
        ) as mock_val,
        patch("stock_data.audit.valuation_audit.run_sw_industry_audit", return_value={}) as mock_sw,
    ):
        res = run_audit(
    ...
    with (
        patch("stock_data.audit.factor_audit.run_adj_factor_audit", return_value={}) as mock_fac,
        patch("stock_data.audit.factor_audit.run_sw_daily_audit", return_value={}) as mock_sw_d,
    ):
        res = run_audit(audit_type="factor", data_source="tushare")
        mock_fac.assert_called_once()
        mock_sw_d.assert_called_once()

    with patch("stock_data.audit.moneyflow_audit.run_hk_hold_audit", return_value={}) as mock_mf:
        res = run_audit(audit_type="moneyflow", data_source="tushare")
        mock_mf.assert_called_once()

    with patch(
        "stock_data.audit.distribution_audit.run_distribution_audit", return_value={"status": "PASSED"}
    ) as mock_dist:
...
def test_main_cli_dispatch() -> None:
    with (
        ...
        patch("stock_cli.audit.run_audit") as mock_run,
    ):
        main()
...
def test_main_cli_error_exit() -> None:
    with (
        ...
        patch("stock_cli.audit.run_audit", side_effect=ValueError("Test Error")),
...
def test_main_cli_exits_on_failed_audit_result() -> None:
    with (
        ...
        patch("stock_cli.audit.run_audit", return_value={"acceptance": {"status": "FAILED"}}),
        pytest.raises(SystemExit) as exc_info,
    ):
        main()
    assert exc_info.value.code == 1
...
def test_main_cli_exits_on_master_audit_errors() -> None:
    ...
    with (
        patch("sys.argv", ["audit.py", "-t", "master"]),
        patch("stock_cli.audit.run_audit", return_value=result),
        pytest.raises(SystemExit) as exc_info,
    ):
        main()
    assert exc_info.value.code == 1
...
def test_resolve_audit_target_date_auto() -> None:
    from stock_cli.audit import _resolve_audit_target_date
    ...
    # 2. 自动探测
    with patch("stock_data.catalog.DataCatalog.latest_trade_dates", return_value=[date(2026, 8, 11)]):
