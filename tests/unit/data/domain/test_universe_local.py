...
def test_universe_filter_local_mode_success(tmp_path: Path, monkeypatch) -> None:
            ...
            records.append(
                {
                    ...
                    "close": 10.5,
                    "volume": 1000.0,
                    "amount": 50000.0 if code == "000001.SZ" else (100000.0 if code == "600519.SH" else 5000.0),
                    "data_source": "tushare",
                    "market": "CN",
    ...
    p_bar = tmp_path / "tushare" / "market=CN" / "stock_daily_bar" / "year=2024" / "month=01" / "data.parquet"
    ...
    p_db = tmp_path / "tushare" / "market=CN" / "daily_basic" / "year=2024" / "month=01" / "data.parquet"
