...
- `config/`：策略、风险、标的池与智能体画像 YAML 配置（`config/universe/watchlist.yaml` 为唯一核心观察池；`config/agent/user_persona.yaml` 为唯一用户投资画像与风控底线；`config/agent/soul.md` 为投研专家人格底色与求真准则）；`data/`：本地 RAW/Curated 2-Tier 离线 Parquet 数据与缓存，不提交敏感信息或临时产物。
...
4. **量化投研与分析准则 (Ground Truth First & 严禁编造)**：
   - 严格遵循**本地真实数据第一**：所有点位、估值与指标必须通过代码查询本地 Curated 黄金表输出，严禁凭模型记忆虚构数值；**本地查不到数据时，必须如实说明缺失，严禁胡乱编造；确需补充时必须通过权威渠道检索并注明来源与时效**；
   - 严格遵循**指标策略权威依据**：**严禁胡乱编造量化指标定义、公式或交易策略**；若公式或金融逻辑不确定，必须先检索外部权威金融文献、交易所/指数公司官方编制方案或券商金工研报，明确阐释依据来源；
   - 严格执行**三层信息分级透明化**（已验证事实 / 机制推断 / 外部背景）与**防过拟合审查**；
   - **严禁无源叙事污染**：本地库无政策或新闻数据表，严禁将外部记忆中的“政策刺激/救市会议”伪装为数据事实写入结论或表格，详见 [`.agents/skills/data-catalog/references/03_data_query_caveats.md`](file:///Users/mac/workspace/personal/finance/stock/.agents/skills/data-catalog/references/03_data_query_caveats.md)。
