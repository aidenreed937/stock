...
6. **外盘时序因果截断与时序解耦规范 (No Look-ahead Bias)**：
   * **回顾归因 (Backward-Looking)**：解释 A 股 T 日白天走势时，外盘参照系**必须严格截断至 T-1 日收盘（由 `source_cutoffs.external_market` 留痕）**，因为 T 日 A 股交易时 T 日美股夜盘尚未发生，严禁以“本地已有数据”为由偷看 T 日夜盘；
   * **次日开盘前瞻 (Forward-Looking)**：T 日美股夜盘收盘数据，专供推演 **T+1 日（如周一）A 股开盘情绪与板块映射**；
   * 研报与对话中必须将“T-1 隔夜事实”与“T 夜盘前瞻”显式分栏呈现。
7. **严禁外盘指标混淆与张冠李戴 (SPASIA50 vs A50 权威辨析)**：
   * **`SPASIA50.SPI` 是标普亚洲 50 指数（S&P Asia 50）**，成份涵盖台积电、三星、丰田、索尼、腾讯等全亚洲巨头，**绝不是新加坡交易所富时中国 A50 指数期货**；
   * 本地数据库（TuShare/yfinance）**无新加坡 SGX A50 原生期货行情表**；
   * 海外离岸中国资产与隔夜前瞻的**唯一权威代理矩阵**为：`FXI`（富时中国50 ETF）、`ASHR`（沪深300离岸 ETF）、`KWEB`（中国互联网 ETF）、`USD/CNH`（离岸人民币汇率 `CNH=X`）与 `^VIX`（恐慌指数）。严禁将不相干指数强行当作 A50。
