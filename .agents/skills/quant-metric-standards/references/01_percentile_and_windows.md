...
| :--- | :--- | :--- | :--- | :--- |
| **5 年滚动窗口 (5Y Rolling)** | $\sim 1,215$ 交易日 | • 个股/行业估值 (PE/PB)<br>• 行业轮动动量<br>• 短中期情绪/换手率 | 中期估值与行业景气度比较 | 覆盖完整的一轮朱格拉/产能库存周期（3~5年），避免过早历史对当前新兴产业或成熟产业结构造成失真。 |
| **10 年滚动窗口 (10Y Rolling)** | $\sim 2,430$ 交易日 | • 大盘宽基估值 (沪深300/中证500)<br>• 股债利差 (ERP)<br>• 长期资产收益率 | 跨越完整牛熊周期的战略配置 | 覆盖完整的康波/信贷大周期（7~10年），包含至少 1~2 次完整牛熊转换，具备极强的统计稳健性。 |
...
$$\text{Percentile}(x_t) = \frac{\text{Rank}(x_t) - 1}{N - 1} \times 100\% \quad (N > 1)$$

* **定义说明**：$\text{Rank}(x_t)$ 为样本升序排序后的名次（最小值排名为 1，最大值排名为 $N$）；
...
SELECT
    ...
    PERCENT_RANK() OVER (
        PARTITION BY symbol
        ORDER BY pe_ttm
        ROWS BETWEEN 2429 PRECEDING AND CURRENT ROW
