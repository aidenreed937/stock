...
> 下表"实测覆盖"均基于 **2025-08-01 ~ 2026-08-20 半年窗口**逐数据集实跑确认。覆盖率与"全市场"存在差距的数据集已在表格注明，**不代表可执行全市场规则**，只能作为样本不足的观察项或直接不支持。
...
stock_screen:
  ...
  symbols: []                # 空 = 全市场；可传 watchlist 限定范围

  hard_exclusion:
    st_name_regex: "ST|\\*ST|退"
    min_list_days: 180
    min_close_price: 2.0                 # 元
    min_daily_amount_yuan: 5_000_0000    # 5000 万元
    consecutive_loss_years: 2            # 连续亏损年数
    max_debt_days_lag: 120               # 财报可用时的最大滞后容忍
    max_goodwill_to_equity: 0.50
    require_trade_on_as_of: true         # 当日停牌则剔除

  yellow_warn:
    forecast_p_change_min_threshold: -50.0      # 业绩预告下修幅度 %
    holder_sell_count_180d: 3                    # 半年减持笔数（in_de=DE）
    north_flow_drop_20d_threshold: -0.20          # 北向 20 日降幅（仅北向标的）
    consecutive_limit_down_count: 2               # 连续跌停
    goodwill_observe_ratio: 0.30                  # 商誉/净资产观察线（高于 0.30 黄牌，高于 0.50 硬剔除）
    # ---- 以下默认关闭，实测覆盖不足，待数据补齐后再启用 ----
    margin_balance_drop_20d_threshold: -0.30      # 两融 20 日降幅（仅 watchlist）
    max_pledge_ratio: 0.70                       # 股权质押比例（仅 6 只快照）
    block_trade_discount_threshold: -0.10         # 大宗折价率（第二期）
  ...
  datasets:                   # 与 industry_structure config 相同的 schema 规范
    ...
    # 确认-标的子集：hk_hold（北向标的约 981 只）、margin_detail（仅 watchlist，默认关闭）
    # 不足默认关闭：lixinger/pledge_info（仅 6 只快照）
    # 缺口 not_supported：审计意见、违规/诉讼、限售解禁、share_float
```

### 3.3 规则引擎设计（`rules.py`）

- 每个排雷点 = 一个**纯函数** `evaluate_<rule>(rows: pl.DataFrame) -> pl.DataFrame`，入参一次加载好的数据集，输出带 `{rule_id}: {pass/warn/fail}` 与 reason 文本的追加列。
- 决策合并（`decision.py`）：任何 `fail` → excluded；`warn` 计数 ≥ 3 → 升级为临时排除（列入 excluded 且知名 reason）；其余 → passed。
...
3. **默认关闭 3 条**（实测覆盖不足，`enabled: false`，不阻塞运行）：两融个股明细（仅 watchlist 7 只）、股权质押（仅 6 只）、限售解禁（本地无表）。
