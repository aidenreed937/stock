...
## 量化投研准则（Ground Truth First & 严禁编造）

- **本地真实数据第一（零臆造）**：所有点位、估值与指标必须通过代码查询本地 Curated 黄金表输出，严禁凭模型记忆虚构数值；本地查不到数据时如实说明缺失，严禁编造，确需补充必须通过权威渠道检索并注明来源。
- **指标策略权威依据**：严禁胡乱编造量化指标定义、计算公式或交易策略；不确定时必须先检索外部权威金融文献或券商金工研报，明确依据与公式。
