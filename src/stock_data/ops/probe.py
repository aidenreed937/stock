...
from stock_data.fetcher.fred import create_fred_fetcher
from stock_data.fetcher.lixinger import LixingerDataFetcher
...
class GlobalDataProbe:
    ...
    def probe_tushare(self) -> list[dict[str, Any]]:
        ...
        start_t = time.monotonic()
        try:
            df = self.tushare_fetcher.fetch_daily_bars_df(
                "600000.SH", target_date, target_date, endpoint="daily_basic"
            )
            lat = (time.monotonic() - start_t) * 1000
            results.append(
                {
    ...
    def probe_yfinance(self) -> list[dict[str, Any]]:
        ...
        start_t = time.monotonic()
        try:
            df = self.yfinance_fetcher.fetch_index_valuations_df(
                etf_map={"SPY": "^GSPC", "QQQ": "^IXIC"}
            )
            lat = (time.monotonic() - start_t) * 1000
            results.append(
                {
        ...
        start_t = time.monotonic()
        try:
            df = self.yfinance_fetcher.fetch_macro_indicators_df(
                target_date, today, symbols=["^TNX", "GC=F", "^VIX"]
            )
            lat = (time.monotonic() - start_t) * 1000
            results.append(
                {
    ...
    def probe_lixinger(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            df = self.lixinger_fetcher.fetch_daily_bars_df(
                "600519", date(2026, 8, 1), date(2026, 8, 10)
            )
            lat = (time.monotonic() - start_t) * 1000
            results.append(
                {
...
def main() -> None:
    ...
    print("=" * 95)
    print(
        "                全数据源 (TuShare, yfinance, FRED, 理杏仁) 健康度与连通性验证报告                "
    )
    print("=" * 95)
    ...
    for r in results:
        ...
        if status == "SUCCESS":
            ...
            cols = r.get("cols", 0)
            print(
                f"[OK]   [{src:<8}] {ep:<30} | 频次: {freq:<9} | 记录: {rows:>4} 条 | 字段: {cols:>2} 列 | 耗时: {lat:>7.2f}ms"
            )
        elif status == "EMPTY":
            print(
                f"[WARN] [{src:<8}] {ep:<30} | 频次: {freq:<9} | 记录:    0 条 | (可能是非交易日或更新未完成) | 耗时: {lat:>7.2f}ms"
            )
        else:
            err = r.get("error", "未知错误/未配置Token")
            print(
                f"[INFO] [{src:<8}] {ep:<30} | 频次: {freq:<9} | 状态: {status:<8} | 提示: {err[:35]}"
            )
