...
from stock_data.normalizer.bar_normalizer import infer_market_exchange_currency
...
def clean_partition_daily_basic(
    ...
    *,
    ...
    if not raw_hyphen.is_empty() and not raw_full.is_empty():
        ...
        hyphen_only = hyphen_with_keys.join(raw_full_keys, on=["_sym", "_date"], how="anti").drop(
            ["_sym", "_date"]
        )
    else:
    ...
    if not cleaned_cur.is_empty():
        ...
        cleaned_cur = cleaned_cur.with_columns(parse_mixed_date(date_col).alias("trade_date"))
    ...
    if "total_mv" in cleaned_cur.columns:
        abnormal_mv_count = int(cleaned_cur.filter(pl.col("total_mv") >= 1e12).height)
...
def repair_all_daily_basic(
    ...
    *,
    ...
    raw_files = (
        sorted([p for p in raw_base.rglob("*.parquet") if not _is_artifact(p)])
        if raw_base.exists()
        else []
    )
