...
def repartition_all_curated(base_dir: str = "data/curated") -> None:
    ...
    for (src, market, dataset), file_list in dataset_groups.items():
        logger.info(
            f"===> 正在处理 [{src}/{market}/{dataset}] (共 {len(file_list)} 个原始分区文件)..."
        )
        dfs = []
        for fp in file_list:
            try:
        ...
        if "exchange" not in merged_df.columns or "currency" not in merged_df.columns:
            ...
            if "symbol" in merged_df.columns:
                _, ex_exp, cur_exp = infer_market_exchange_currency(
                    pl.col("symbol"), data_source=src
                )
            elif "ts_code" in merged_df.columns:
                _, ex_exp, cur_exp = infer_market_exchange_currency(
                    pl.col("ts_code"), data_source=src
                )
            else:
...
def repartition_dataset(
    ...
    try:
        ...
        staged_frame = pl.concat(
            [pl.read_parquet(path) for path in staged_files], how="diagonal_relaxed"
        )
