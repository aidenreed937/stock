...
def _snake_case_column_name(name: object) -> str:
    """将 Yahoo 财报标题式或驼峰式字段转换为标准 snake_case。"""
    text = re.sub(r"([a-z0-9])([A-Z])", r"\1_\2", str(name))
    text = re.sub(r"([A-Z]+)([A-Z][a-z])", r"\1_\2", text)
    return re.sub(r"[^0-9A-Za-z]+", "_", text).strip("_").lower()


def _normalize_financial_statement_columns(df: Any) -> Any:
    """归一化 yfinance 财报字段名，保留财报的期间行粒度。"""
    date_column = next(
        (column for column in ("asOfDate", "as_of_date", "Date", "index") if column in df.columns),
        None,
    )
    if date_column is not None and date_column != "as_of_date":
        df = df.rename(columns={date_column: "as_of_date"})
    return df.rename(columns={column: _snake_case_column_name(column) for column in df.columns})
...
class YFinanceDataFetcher(BaseDataFetcher):
    ...
    def _fetch_ticker_table_df(self, symbol: str, endpoint: str) -> pl.DataFrame:
        ...
        raw = self.client.query_ticker(
            symbol, lambda ticker: getattr(ticker, attr_map[endpoint], None)
        )
    ...
    def fetch_index_valuations(
        ...
        for etf_symbol, target_index in mapping.items():
            info = self.client.query_ticker(etf_symbol, lambda ticker: ticker.info or {})
    ...
    def fetch_financials_df(
        ...
        df_raw = self.client.query_ticker(symbol, lambda ticker: getattr(ticker, attr_name, None))
        if df_raw is None or df_raw.empty:
            return pl.DataFrame()

        # yfinance 返回的报表列是日期，索引是科目
        reset_df = _normalize_financial_statement_columns(df_raw.T.reset_index())
    ...
    def fetch_actions_df(self, symbol: str, action_type: str = "dividends") -> pl.DataFrame:
        """抓取公司历史分红派息或拆股记录。"""
        series_raw = self.client.query_ticker(
            symbol, lambda ticker: getattr(ticker, action_type, None)
        )
    ...
    def fetch_analyst_target_df(self, symbol: str) -> pl.DataFrame:
        ...
        targets = self.client.query_ticker(
            symbol, lambda ticker: getattr(ticker, "analyst_price_target", None)
        )
    ...
    def fetch_fast_info_df(self, symbol: str) -> pl.DataFrame:
        ...
        def build_record(ticker: Any) -> dict[str, Any]:
        ...
        return pl.DataFrame([self.client.query_ticker(symbol, build_record)])
