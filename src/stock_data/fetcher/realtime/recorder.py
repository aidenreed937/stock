...
class RealtimeSnapshotRecorder:
    """按日期/小时分区批量追加实时快照，不写入 Curated 黄金表。"""
    ...
    def __init__(
        ...
        *,
        ...
        if runtime is None:
            ...
            context = runtime
        self.root = Path(root) if root is not None else context.raw_root / "realtime" / source
    ...
    def append(
        ...
        *,
        ...
        """追加一批快照；达到 flush 间隔或 force 时写入 Parquet。"""
        self._pending.extend(self._record_for(quote) for quote in quotes)
    ...
    @staticmethod
    def _record_for(quote: RealtimeQuote) -> dict[str, object]:
        record = quote.model_dump()
        record["bid_prices"] = list(quote.bid_prices)
        record["bid_volumes"] = list(quote.bid_volumes)
        record["ask_prices"] = list(quote.ask_prices)
        record["ask_volumes"] = list(quote.ask_volumes)
        return record


__all__ = ["RealtimeSnapshotRecorder"]
