...
from stock_data.governance.audit.benchmarks.base import get_trading_calendar
from stock_data.governance.audit.domains import AuditReportResult
from stock_data.governance.audit.registry import get_audit_spec, resolve_benchmark_provider
...
def extract_identity_keys(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    sym_cols = [
        c for c in ["symbol", "ts_code", "index_code", "industry_code"] if c in frame.columns
    ]
...
class UniversalAuditEngine:
    ...
    def audit_single_day(
        ...
        extra_samples = [
            f"{r['symbol']}@{r['trade_date']}" for r in extra_df.head(10).iter_rows(named=True)
        ]
