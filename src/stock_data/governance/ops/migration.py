...
def _primary_keys(path: Path, columns: list[str]) -> list[str]:
    ...
    if endpoint in BAR_DATASETS:
        ...
        return [column for column in ("market", "symbol", "trade_date") if column in columns]
...
def _repair_lineage(df: pl.DataFrame, path: Path) -> tuple[pl.DataFrame, bool]:
    ...
    endpoint = _endpoint_name(path)
    partition = "/".join(part for part in path.parts if part.startswith(("year=", "month=")))
    ...
    if "updated_at" not in repaired.columns:
        repaired = repaired.with_columns(pl.lit(None, dtype=pl.Datetime).alias("updated_at"))
        changed = True
...
def _dedupe_frame(df: pl.DataFrame, keys: list[str]) -> pl.DataFrame:
    ...
    work = work.with_columns(
        pl.col("_migration_updated_at").fill_null(0).alias("_migration_updated_sort")
    )
...
def migrate_parquet(
    ...
    base = Path(root)
    paths = (
        [path for path in base.rglob("*.parquet") if not _is_artifact(path)]
        if base.exists()
        else []
    )
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {"before_files": len(before["files"]), "after_files": len(after["files"]), **result},
            ensure_ascii=False,
            indent=2,
        )
    )
