...
from stock_core.contracts import InstrumentId
...
_CURATED_BAR_STRING_COLUMNS = frozenset(
    set(CURATED_STOCK_DAILY_BAR_COLUMNS) - _CURATED_BAR_FLOAT_COLUMNS - {"trade_date", "updated_at"}
)
...
def infer_metadata_expressions(
    ...
    if data_source in {"alphavantage", "yfinance"} and (
        dataset == "macro_indicators" or api_name in {"macro_indicators", "FX_DAILY"}
    ):
