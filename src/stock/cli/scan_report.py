...
def format_investor_report(data_or_summary: DailyMarketScanSummary | dict[str, Any]) -> str:
    ...
    # 2. 信号表格
    signal_rows = [
        f"| {s.category} | {s.name} | {s.value_str} | {s.percentile_str} | "
    ...
    lines = [
        ...
        *signal_rows,
        ...
        "",
        "## 微观健康度",
        f"- 两融杠杆：**{mh.margin_ratio:.2f}%**（{mh.margin_status}）",
...
def _render_pro_macro_section(summary: DailyMarketScanSummary, dt_str: str) -> list[str]:
    ...
    eyby_s = (
        f"{eyby.ey_by_ratio:.2f}x (PE: {eyby.pe_ttm:.2f}, "
        f"10Y: {eyby.bond_yield_10y:.3f}%, Pctl: {eyby.percentile_10y:.1f}%)"
        if eyby
        else "N/A"
    )
    all_m_s = (
        f"PB: {all_m.pb_ew:.3f} (Pctl: {all_m.pb_percentile_10y:.1f}%), "
        f"PE: {all_m.pe_ttm_ew:.2f} (Pctl: {all_m.pe_percentile_10y:.1f}%)"
        if all_m
        else "N/A"
    )
    buf_s = (
        f"{buffett.securitization_ratio:.1f}% (MV: {buffett.total_market_cap_yi:.0f}亿, "
    ...
    lines = [
        f"## 1. 宏观周期与大类资产定价 (Macro Regime, As-Of: {macro_dt})",
...
def _render_pro_industry_section(summary: DailyMarketScanSummary, dt_str: str) -> list[str]:
    ...
    return [
        f"## 2. 中观行业轮动与风控 (TCR As-Of: {tcr_dt}, PB-ROE As-Of: {pbroe_dt})",
