"""TuShare 门面 (Facade) 数据抓取器实现。"""

from datetime import date
from typing import Any

import polars as pl

from stock.data.fetcher.base import BaseDataFetcher
from stock.data.fetcher.tushare.client import TuShareClient
from stock.data.fetcher.tushare.stock_fetcher import TuShareStockFetcher
from stock.models.market import DailyBar


class TuShareDataFetcher(BaseDataFetcher):
    """TuShare 统一门面数据抓取器，实现 BaseDataFetcher 抽象接口。"""

    def __init__(
        self, token: str | None = None, url: str | None = None
    ) -> None:
        """初始化 TuShareDataFetcher 门面。

        Args:
            token: TuShare API Token。若为 None，则自动从配置文件读取。
            url: TuShare API 服务器地址。若为 None，则自动从配置文件读取。
        """
        self.client = TuShareClient(token=token, url=url)
        self.stock_fetcher = TuShareStockFetcher(client=self.client)

    def fetch_daily_bars(
        self, symbol: str, start_date: date, end_date: date
    ) -> list[DailyBar]:
        """抓取日 K 线并转换为 DailyBar 模型列表。"""
        return self.stock_fetcher.fetch_daily_bars(symbol, start_date, end_date)

    def fetch_daily_bars_df(
        self,
        symbol: str,
        start_date: date,
        end_date: date,
        endpoint: str = "daily",
        **kwargs: Any,
    ) -> pl.DataFrame:
        """抓取行情或基本面数据，并返回 Polars 数据帧。"""
        return self.stock_fetcher.fetch_daily_bars_df(
            symbol, start_date, end_date, endpoint=endpoint, **kwargs
        )

    def fetch_trade_cal(
        self, start_date: date, end_date: date
    ) -> list[date]:
        """获取 A 股有效开市交易日列表。"""
        return self.stock_fetcher.fetch_trade_cal(start_date, end_date)
