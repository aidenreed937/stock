...
class DatasetKey:
    ...
    def market_slug(self) -> str:
        """返回用于 Hive 目录划分的市场标识 (优先使用标的市场，否则根据数据源规则推断)。"""
        ...
        if provider_lower == "tushare":
            market = "CN"
        elif provider_lower in {"yfinance", "fred"}:
            market = "US"
        elif provider_lower == "lixinger":
            market = "HK" if self.endpoint.lower().startswith("hk") else "CN"
